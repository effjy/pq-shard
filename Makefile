# Makefile for PQ-Shard - post-quantum Shamir secret sharing.
#
#   make                build ./pqshard-gui (GTK3 app) and ./pqshard (CLI)
...
CFLAGS  += -DPQSHARD_VERSION=\"$(VERSION)\" -DKYBER_K=4 -Isrc/kyber
...
# Shared crypto core (no GTK): Shamir engine, share format, hybrid KEM.
CORE_SRC = src/shamir.c src/share.c src/hybrid_kem.c $(KYBER_SRC)
...
CLI_LIBS   = $(shell pkg-config --libs $(CLI_PKGS))
...
install: all
	...
	install -m 0644 data/pqshard.svg $(DESTDIR)$(ICONDIR)/pqshard.svg
	for s in $(ICON_SIZES); do \
	    install -d $(DESTDIR)$(ICONBASE)/$${s}x$${s}/apps; \
	    install -m 0644 data/pqshard-$${s}.png \
	        $(DESTDIR)$(ICONBASE)/$${s}x$${s}/apps/pqshard.png; \
	done
	install -d $(DESTDIR)$(APPDIR)
	install -m 0644 data/pqshard.desktop $(DESTDIR)$(APPDIR)/pqshard.desktop
	...
	@echo "PQ-Shard $(VERSION) installed to $(BINDIR)"

uninstall:
	rm -f $(DESTDIR)$(BINDIR)/$(GUI)
	rm -f $(DESTDIR)$(BINDIR)/$(CLI)
	rm -f $(DESTDIR)$(ICONDIR)/pqshard.svg
	for s in $(ICON_SIZES); do \
	    rm -f $(DESTDIR)$(ICONBASE)/$${s}x$${s}/apps/pqshard.png; \
	done
	rm -f $(DESTDIR)$(APPDIR)/pqshard.desktop
	-update-desktop-database $(DESTDIR)$(APPDIR) 2>/dev/null || true
	-gtk-update-icon-cache -f -t $(DESTDIR)$(DATADIR)/icons/hicolor 2>/dev/null || true
	@echo "PQ-Shard uninstalled"
